        ...
        if(row >= 1 && row <= len) {
            ...
            if(row >= 1 && row <= len) {
                if(check_coordinates(len, table, all_turns, turn, row - 1, col - 1)) {
                    /** TODO BES do you mean to add valid = true? cause i don't see where you turn the variable to true.
                    maybe you want to intentionally make an infinite loop that breaks only with return but this is not the accepted way to do this.
                    Compilers can't figure out infinite loops, it is generally known as the Halting Problem (HP). And in this case the compiler
                    doesn't know you are breaking the loop with the returns, so it complains that in the end of the function you are not returning anything, that's why I added 
                    the return -1 in the end. Fix it **/
                    return turn;
                }
            }
            else {
...
int main()
{
	...
	for(int turn = 1; turn <= max_turn_num; turn++)
    {
        ...
        printf("\n%d %d\n", all_turns[turn - 1][ROW], all_turns[turn - 1][COL]);
         // Output: True values
